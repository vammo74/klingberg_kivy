...
class MyButton(Button):
	#but_ids = DictProperty({})
#	def __init__(self, **kwargs):
#	       super(MyButton, self).__init__(**kwargs)
#	       Clock.schedule_once(self._get_but_ids)
#	       for nyckel, objekt in self.but_ids:
#	       	print(nyckel, objekt)
#	def _get_but_ids(self, *args):
#		_id = "kbtn" + self.text
#		self.but_ids[_id] = self
#		if self.text in ['1', '2', '3', '4']:
#			keybinding.start(int(self.text)[, self)
#		print(_id)
#		print(self)
#		for nyckel, objekt in self.but_ids:
#			print(nyckel, objekt)
				...
				if app.disp_ids['dbtn1'].text == str(app.integer_answer()):
					...
					if app.level == 0:
						...
						if len(app.product_list) == 0:
							self.analyse()
							app.product_list = app.generate_products()
#							print('new level', app.product_list, len(app.product_list))
							app.stop_timer()
							app.level = app.generate_level()
							print('level', app.level)
							app.table.table_on()
	...
	def analyse(self, *largs):
		...
		for key, time in times_orders:
			if isinstance(time, int):
				analysis += "You answered " + key + " correctly in " + str(20*time//360) + " seconds\n"
...
class Klingberg(RelativeLayout):
	...
	def __init__(self, **kwargs):
		...
		Window.bind(on_keyboard_down=self._on_keyboard)
	...
	def _on_keyboard(self, keyboard, key, scancode, codepoint, modifier ):
		...
		print("keyboard pressed {}".format(codepoint))
		if key == '49' :
			print('yes')
			self.button = Button(text = '1')
			app.my_button._on_press(self.button)
	...
	def _start(*args):
		...
		if app.disp_ids['dbtn0'].state == 'down':
			...
			if app.level == 0:
				...
				print('start', app.product_list, len(app.product_list))
			elif app.level > 8:
				print(app.tid)
				app.table.table_off()
				app.start_timer()	
	...
	def load_state(*args):
		...
		try:
			...
			app.level = 0
			app.counter = 0
		print('load state', app.product_list, len(app.product_list))
		if app.product_list == ['']:
			app.product_list = app.generate_products()
		if app.factors == ['']:
			app.factors = []
	...
	def save_state(*args):
		...
		print('save state', app.product_list, len(app.product_list))
		fob.write(";")
...
class KlingbergApp(App):
	...
	def generate_products(self, *args):
		...
		print(productList, self.add_on_products)
	...
	def generate_level(self, *args):
		...
		if wrong['x2'] >= 3: return 1
		if wrong['x3'] >= 3: return 2
		if wrong['x4'] >= 3: return 3
		if wrong['x5'] >= 2: return 4
		if wrong['x6'] >= 2: return 5
		if wrong['x7'] >= 2: return 6
		if wrong['x8'] >= 2: return 7
		if wrong['x9'] >= 2: return 8
		else: return 9
